...
export default function EcosystemSection() {
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-8 items-start mb-12">
          ...
          <div className="md:col-span-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-sm text-muted-foreground font-medium">Ecosystem</div>
                <div className="text-xl font-bold text-foreground">
                  THE SAFEGREY <span className="text-[#FDA172]">ECOSYSTEM</span>
                </div>
              </div>
            </div>
          </div>
  ...
  >
    ...
    {tab.label}
                    ...
                    <div className="relative h-1 bg-gray-600 rounded-full overflow-hidden">
                      <motion.div
                        className={cn(
                          ...
                          isActive ? "bg-white" : isCompleted ? "bg-gray-400" : "bg-transparent"
                        )}
                        initial={{ width: isActive ? "0%" : isCompleted ? "100%" : "0%" }}
                        animate={{
        ...
        {/* Main Content - Description Left, 3D Visualization Right */}
        ...
        {showDetect && (
          ...
          >
            <IsometricLayer
              zIndex={10}
              accent="hsl(45, 93%, 47%)"
              fill="linear-gradient(135deg, hsl(45 93% 47% / 0.08) 0%, hsl(45 93% 47% / 0.15) 100%)"
              dashed
              elevated
              depth={-20}
              offsetY={5}
...
function ProtectSignalBoards() {
  const items = [
    { x: "15%", y: "25%", label: "ASSESS", color: "hsl(142, 76%, 36%)", tooltip: "Comprehensive Vulnerability Assessments" },
    { x: "40%", y: "60%", label: "HARDEN", color: "hsl(142, 76%, 36%)", tooltip: "Infrastructure Hardening & Remediation" },
    { x: "69%", y: "28%", label: "SECURE", color: "hsl(142, 76%, 36%)", tooltip: "Cloud & Network Security" },
  ]
...
function DetectSignalBoards() {
  const items = [
    { x: "15%", y: "65%", label: "MONITOR", color: "hsl(45, 93%, 47%)", tooltip: "Continuous SIEM Monitoring" },
    { x: "40%", y: "20%", label: "PHISH", color: "hsl(45, 93%, 47%)", tooltip: "Targeted Phishing Campaigns" },
    { x: "70%", y: "65%", label: "HUNT", color: "hsl(45, 93%, 47%)", tooltip: "Threat Hunting & Red Team Exercises" },
  ]
...
function RespondSignalBoards() {
  const items = [
    { x: "20%", y: "28%", label: "VALIDATE", color: "hsl(0, 84%, 60%)", tooltip: "Incident Response Validation" },
    { x: "45%", y: "65%", label: "SIMULATE", color: "hsl(0, 84%, 60%)", tooltip: "Assumed Breach Scenarios" },
    { x: "70%", y: "30%", label: "COLLAB", color: "hsl(0, 84%, 60%)", tooltip: "Purple Team Collaboration" },
  ]
...
function SignalBoard({
  x, y, label, color, tooltip, delay = 0, height = 60, stickHeight = 30, elevated = false
}: {
  x: string;
  y: string;
  label: string;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ height: `${stickHeight}px`, bottom: `${height}px`, left: '50%', transform: 'translateX(-50%)' }}
      />
  ...
  return (
    <div className="absolute inset-0">
      {nodes.map((node, i) => (
        <motion.div
          key={i}
          className={`absolute rounded-full bg-red-500/40 ${node.size === 'small' ? 'w-3 h-3' :
              node.size === 'medium' ? 'w-4 h-4' : 'w-5 h-5'
            }`}
          style={{
            left: node.x,
            top: node.y,
...
function AnimatedMatrix() {
  const rows = 8
  const cols = 20
  const values = Array.from({ length: rows * cols }, () =>
    Math.random() > 0.7 ? Math.floor(Math.random() * 2) : Math.floor(Math.random() * 10)
  )
